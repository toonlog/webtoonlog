...
export async function GET(request) {
  try {
    ...
    if (userId && !collectionId) {
      ...
      const collections = await Promise.all(records.map(async r => {
        try {
          const c = await base('COLLECTION').find(r.fields.collection_id);
          return { id: c.id, name: c.fields.name, is_public: c.fields.is_public, user_id: c.fields.user_id };
        } catch { return null; }
      }));
